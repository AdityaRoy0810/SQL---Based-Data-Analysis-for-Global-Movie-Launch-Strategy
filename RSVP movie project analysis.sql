...
SELECT SUM(CASE WHEN title IS NULL THEN 1 ELSE 0 END) AS title_null_count, 
...
SELECT COUNT(*) AS number_of _movies
FROM movie
GROUP BY year; 
...
SELECT 
      ...
      COUNT(CASE WHEN known_for_movies IS NULL THEN id END) AS known_for_movie_nulls
FROM names;
